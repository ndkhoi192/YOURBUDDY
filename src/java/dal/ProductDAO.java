...
public class ProductDAO extends DBContext {
    ...
    public void addProduct(Product product) {
        String sql = "INSERT INTO Product (productName, price, description, cateID, petID, imageURL, discount, stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement pre = connection.prepareStatement(sql)) {
            pre.setString(1, product.getProductName());
            pre.setDouble(2, product.getPrice());
            pre.setString(3, product.getDescription());
            pre.setInt(4, product.getCateID());
            pre.setInt(5, product.getPetID());
    ...
    public boolean updateProduct(Product product) {
        String sql = "UPDATE Product SET productName=?, price=?, description=?, cateID=?, petID=?, imageURL=?, discount=?, stock=? WHERE productID=?";
        try (PreparedStatement pre = connection.prepareStatement(sql)) {
            pre.setString(1, product.getProductName());
            pre.setDouble(2, product.getPrice());
            pre.setString(3, product.getDescription());
    ...
    public List<Product> getSimilarProducts(int cateID, int petID, int currentProductID, int limit) {
        List<Product> products = new ArrayList<>();
        String sql = "SELECT TOP (?) * FROM Product WHERE cateID = ? AND petID = ? AND productID != ?";
        try (PreparedStatement pre = connection.prepareStatement(sql)) {
            pre.setInt(1, limit);
            pre.setInt(2, cateID);
            pre.setInt(3, petID);
